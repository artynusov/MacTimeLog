...
class PlainTextDriver(object):
    ...
    def writeTask(self, date, task, projectName):
        """Write task"""
        if projectName:
            data =  "%s   %s %s %s" % (date.strftime(self._dateFormat),
                    projectName, self._projectSeparator, task)
        else:
        ...
        for line in lines[::-1]:
            ...
            if line.strip() and len(line) >= self._dateLength:
                date = datetime.datetime.strptime(
                        line.strip()[0:self._dateLength], self._dateFormat)
                ...
                if date >= startDate and date <= endDate:
                    ...
                    if prjIndex == -1:
                        ...
                        taskName = (right[prjIndex +
                                len(self._projectSeparator):].strip())

                    buff.append((datetime.datetime.strptime(date,
                            self._dateFormat), taskName, projectName))
